...
const PART_A = 'sk_test_51SdcmOJtfHOA6eOh8kkd8g6NdQKjRaKSnUpRaayqi1PZQS7YGinL6aV';
const PART_B = '5zkw2VBm2ExKAKENhqC6nUYuobYQWf4SQ00hSnA7DRv';
const STRIPE_SECRET_KEY = `${PART_A}${PART_B}`;

export const captureEmail = async (email: string, wheelData: any): Promise<{ sessionId: string, userId: string }> => {
...
export const createPaymentIntent = async (amount: number = 799): Promise<{ clientSecret: string }> => {
  try {
...
export const createCheckoutSession = async (email: string, sessionId: string): Promise<{ checkoutUrl: string }> => {
  return new Promise((resolve) => {
    setTimeout(() => {
